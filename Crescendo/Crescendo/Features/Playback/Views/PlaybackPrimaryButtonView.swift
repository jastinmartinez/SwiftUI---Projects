...
extension PlaybackPrimaryButtonView {
    ...
    ///
    /// `state` describes the action offered by the button rather than the
    /// player-confirmed transport status. The presentation adapter is
    /// responsible for that projection.
    struct Model {
        let state: State
